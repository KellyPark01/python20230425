...
for n in range(0,10):  # 0부터 9까지의 정수를 만들어준다. 규칙이 있는 수열이므로 range 사용
        ...
        req = urllib.request.Request(data, \
                                    headers = hdr)
        data = urllib.request.urlopen(req).read()   # data 분자열 변수
        #한글이 깨지는 경우 다시 해석
        page = data.decode('utf-8', 'ignore')  # data는 문자열 변수이고 utf-8 유니코드로 다시 해석하고 ignore 글자가 깨져도 상관없다. 
        #2300줄 문자열 넘기는 코드
        soup = BeautifulSoup(page, 'html.parser')
        list = soup.find_all('span', attrs={'data-role':'list-title-text'})   # span 태그 찾기
