...
soup = BeautifulSoup(html, 'html.parser')  # html 변수

#id와 class로 찾기
# #뒤에 id를 입력해서 찾거나 .뒤에 class를 넣는 방법이 있다. 
# ids_notebook1 = soup.select('#notebook1')  # # = id로 매핑,  id = notebook1 인것을 가져옴, select 검색 메소드
# print(ids_notebook1)
# class_price = soup.select('.price')  # . = class로 매핑 class = price
# print(class_price)
...
# #이때 맥북프로를 제외하고 맥북에어만 찾기 위해 맥북에어가 
# 포함된 부모 태그 정보를 추가한다. 
# tags_notebook1 = soup.select('#notebook1 > span.name') # #은 id를 매핑하므로 id = notebook1 (부모), span 태그의 클래스가 name 인 것만 가져온다 ( . = class를 매핑)
...
tags_notebook2 = soup.select('div.sale > #notebook1 > span.name')  # div 태그의 class = sale,  그 안에 id = notebook1, 그 안에 span 태그 안에 class = name 인 것
tags_notebook3 = soup.select('div.sale span.name')  # div 태그의 class = sale 이고 그 안에 뭐든 다 좋고 span 태그 안에 class = name 인 것
